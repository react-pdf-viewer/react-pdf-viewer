...
test('Keep current position after zooming out', async () => {
    ...
    await page.evaluate(() => (document.querySelector('[data-testid="core__inner-pages"]').scrollTop = 126788));
    ...
    const zoomTo150Button = zoomButtons[7];
...
test('Keep current position after zooming to special level', async () => {
    ...
    // Zoom to fit page
    const zoomToFitPageButton = zoomButtons[1];
    ...
    const zoomToFitWidthButton = zoomButtons[2];
